...
typedef struct tagRootfsPartInfo
{
    char PackName[32];
...
static PART_INFO rootfs_infos[] = {
    {"rootfs_uboot.bin", 0x0, 0x40000},
    {"rootfs_env", 0x40000, 0x10000},
    {"rootfs_kernel.img", 0x50000, 0x240000},
    {"rootfs_romfs.img", 0x290000, 0x680000},
    {"rootfs_web.img", 0x910000, 0x200000},
    {"rootfs_custom.img", 0xB10000, 0x60000},
    {"rootfs_config.img", 0xB70000, 0xC0000},
    {"rootfs_onvif.img", 0xC30000, 0x3D0000},
};
...
int main(int argc, char *argv[])
{
    ...
    if (bexists) {
        ...
        int i = 0;
        for (i = 0; i < sizeof(rootfs_infos)/sizeof(rootfs_infos[0]); ++i) {
            int begin_pos = rootfs_infos[i].ImageOffset;
            int total_size = rootfs_infos[i].ImageSize;
            int pos_size = 0;
            int read_len = 0;
            int readed_len = 0;
            int left_len = total_size;
            fs::file out(rootfs_infos[i].PackName, 'w');
            char big_buf[128*1024];
                   
            bin_file.seek(begin_pos);
